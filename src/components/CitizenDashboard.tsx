...
export function CitizenDashboard({ onBack }: CitizenDashboardProps) {
  ...
  const MobileNavItem = ({ view, icon: Icon, label }: { view: ViewState, icon: any, label: string }) => (
    <button
      onClick={() => setCurrentView(view)}
      className={`flex flex-col items-center justify-center w-full h-full transition-colors relative z-10 ${
  ...
  return (
    ...
    <div className="h-[100dvh] w-full max-w-[100vw] bg-gray-50 flex font-sans text-gray-800 selection:bg-emerald-100 overflow-hidden">
      ...
      {/* --- MAIN LAYOUT --- */}
      <main className="flex-1 lg:ml-72 flex flex-col h-full relative w-full max-w-full">
           ...
           <div className="grid grid-cols-5 h-full items-center relative">
               ...
               <div className="absolute -top-6 left-1/2 -translate-x-1/2 z-50">
                  <button 
                    ...
                    className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg shadow-emerald-500/40 transition-transform active:scale-95 border-[4px] border-gray-50 ${
