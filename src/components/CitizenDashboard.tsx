...
import { 
  ...
  LogOut, Bell, ChevronRight, Star, Leaf,User,
...
export function CitizenDashboard({ onBack }: CitizenDashboardProps) {
  ...
  const MobileNavItem = ({ view, icon: Icon, label }: { view: ViewState, icon: any, label: string }) => (
    ...
    >
      <div className={`p-1.5 rounded-xl mb-1 transition-all ${currentView === view ? 'bg-emerald-50' : ''}`}>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex font-sans text-gray-800 selection:bg-emerald-100">
      ...
      <main className="flex-1 lg:ml-72 flex flex-col h-screen relative">
        ...
        {/* --- TICKER BAR --- */}
        <div className="bg-emerald-900 text-emerald-50 text-xs md:text-sm py-2 px-4 flex items-center shadow-md relative z-20 border-b border-emerald-800">
          <div className="flex items-center gap-2 bg-emerald-800/80 backdrop-blur-sm px-3 py-0.5 rounded-md mr-4 z-20 font-bold text-[10px] md:text-xs uppercase tracking-wider shrink-0 border border-emerald-700/50 shadow-inner">
          ...
          <div className="overflow-hidden relative w-full mask-gradient-x">
             ...
             <div className="animate-marquee cursor-default">
               {headlines.map((h, i) => <span key={i} className="mx-8 opacity-90 hover:opacity-100 transition-opacity">• {h}</span>)}
             </div>
          </div>
        </div>

        {/* --- STICKY HEADER --- */}
        <header className="sticky top-0 z-10 px-4 md:px-8 py-4 bg-white/80 backdrop-blur-xl border-b border-gray-200/60 flex justify-between items-center transition-all">
           <div>
           ...
           <div className="flex items-center gap-3 md:gap-5">
              {/* Points Pill */}
              <div className="hidden md:flex items-center gap-2 bg-amber-50 px-3 py-1.5 rounded-full border border-amber-100 text-amber-700 text-sm font-bold shadow-sm">
                 <Star className="w-4 h-4 fill-amber-500 text-amber-500" />
                 <span>{userProfile.points}</span>
              </div>

              {/* Notification Bell */}
              <button className="relative p-2.5 bg-white hover:bg-gray-50 rounded-full border border-gray-200 transition-colors group">
              ...
              <div className="flex items-center gap-3 pl-3 border-l border-gray-200">
                 ...
                 <img 
                   ...
                   alt="User" 
                   className="w-10 h-10 rounded-full border-2 border-white shadow-md bg-gray-100 cursor-pointer hover:ring-2 hover:ring-emerald-500/50 transition-all" 
                 />
              </div>
           </div>
        </header>
        ...
        {/* --- SCROLLABLE CONTENT --- */}
        <div className="flex-1 overflow-y-auto p-4 md:p-8 relative z-0 pb-24 lg:pb-8 custom-scrollbar">
           <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-500">
              ...
              {currentView === 'map' && <div className="h-[65vh] rounded-3xl overflow-hidden border border-gray-200 shadow-xl"><WasteMap viewType="citizen" /></div>}
        ...
        <div className="lg:hidden fixed bottom-0 w-full bg-white border-t border-gray-200 pb-safe pt-2 px-6 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-40 flex justify-between items-end h-20">
           <MobileNavItem view="overview" icon={LayoutDashboard} label="Home" />
           <MobileNavItem view="map" icon={MapIcon} label="Map" />
           
           {/* Floating FAB for Report */}
           <div className="relative -top-6">
              <button 
                onClick={() => setCurrentView('report')}
                className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg shadow-emerald-500/40 transition-transform active:scale-95 ${
                  currentView === 'report' ? 'bg-gray-900 text-white' : 'bg-emerald-600 text-white'
                }`}
              >
